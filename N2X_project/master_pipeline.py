"""
================================================================================
N2X Model — MASTER PIPELINE (3D High-Precision Standardized Formulation)
================================================================================
Runs the complete 3D pipeline end-to-end:
    Step 1: Generate 3D dataset (3D Brownian dynamics simulation with N=2500)
    Step 2: Train neural network (13 inputs → 128 → 128 → 64 → 6 targets)
    Step 3: Evaluate & compare with 3D UCM / FENE-P
    Step 4: Generate standardized 3D plot suite

Run: python N2X_project/master_pipeline.py
================================================================================
"""
...
print("="*70)
print("  N2X 3D — Machine Learning Modeling of Non-Newtonian Fluids")
print("="*70)
...
N_DUMBBELLS = 2500         # ensemble size (higher N reduces stochastic noise)
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 2 — NEURAL NETWORK TRAINING (3D)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "─"*60)
print("  STEP 2: Training 3D N2X neural network …")
print("─"*60)
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 3 — EVALUATION (3D)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "─"*60)
print("  STEP 3: 3D Evaluation …")
print("─"*60)
...
def tr_mse(Ca, Cb):
    return float(np.mean((np.trace(Ca, axis1=1, axis2=2) - np.trace(Cb, axis1=1, axis2=2))**2))

print(f"\n  3D Trajectory tr(C) MSE vs Reference:")
print(f"    N2X NN (3D) : {tr_mse(C_nn_traj,  C_ref_traj):.5f}")
print(f"    UCM (3D)    : {tr_mse(C_ucm_traj, C_ref_traj):.5f}")
print(f"    FENE-P (3D) : {tr_mse(C_fp_traj,  C_ref_traj):.5f}")


# ═══════════════════════════════════════════════════════════════════════════════
# STEP 4 — VISUALISATION (3D)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "─"*60)
print("  STEP 4: Generating 3D plots …")
print("─"*60)
...
fig.suptitle('N2X 3D — Training Curve', fontweight='bold', y=1.01)
plt.tight_layout(); plt.savefig('results/plots/fig1_training_loss.png'); plt.close()

# Fig 2: 3D Parity plots
lbls = [r'$\Omega_{xx}$', r'$\Omega_{xy}$', r'$\Omega_{xz}$',
        r'$\Omega_{yy}$', r'$\Omega_{yz}$', r'$\Omega_{zz}$']
fig, axes = plt.subplots(2, 3, figsize=(15, 9))
for i, (ax, lb) in enumerate(zip(axes.flatten(), lbls)):
...
fig.suptitle('N2X 3D — Parity Plots', fontweight='bold', y=1.01)
plt.tight_layout(); plt.savefig('results/plots/fig2_parity.png'); plt.close()

# Fig 3: Tensor trace 3D
def tr(C): return np.trace(C, axis1=1, axis2=2)
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(time_arr, tr(C_ref_traj), color=COL['ref'],   lw=2.5, label='BD Reference (3D)')
ax.plot(time_arr, tr(C_nn_traj),  color=COL['nn'],    lw=2,   label='N2X NN (3D)')
ax.plot(time_arr, tr(C_ucm_traj), color=COL['ucm'],   lw=2,   ls='--', label='UCM (3D)')
ax.plot(time_arr, tr(C_fp_traj),  color=COL['fenep'], lw=2,   ls=':',  label='FENE-P (3D)')
ax.set_xlabel('Time'); ax.set_ylabel('tr(C)')
ax.set_title(r'3D Conformation Tensor Trace  tr$(C)$  — Shear $\dot\gamma=1$'); ax.legend()
...
for comp, idx_, nm in [((0, 1), r'\tau_{xy}', 'shear'), ((0, 0), r'\tau_{xx}', 'normal')]:
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(time_arr, tau_ref[:, comp[0], comp[1]], color=COL['ref'], lw=2.5, label='BD Reference (3D)')
    ax.plot(time_arr, tau_nn[:, comp[0], comp[1]],  color=COL['nn'],  lw=2,   label='N2X NN (3D)')
    ax.plot(time_arr, tau_ucm[:, comp[0], comp[1]], color=COL['ucm'], lw=2,   ls='--', label='UCM')
    ax.set_xlabel('Time'); ax.set_ylabel(f'{nm} stress')
    ax.set_title(f'Polymer Stress ({nm}) 3D  gdot=1'); ax.legend()
    plt.tight_layout()
    plot_name = 'fig4_stress_shear.png' if nm == 'shear' else 'fig5_stress_normal.png'
    plt.savefig(f'results/plots/{plot_name}'); plt.close()

# Fig 6: Cxx
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(time_arr, C_ref_traj[:, 0, 0], color=COL['ref'], lw=2.5, label='BD Reference (3D)')
ax.plot(time_arr, C_nn_traj[:, 0, 0],  color=COL['nn'],  lw=2,   label='N2X NN (3D)')
ax.plot(time_arr, C_ucm_traj[:, 0, 0], color=COL['ucm'], lw=2,   ls='--', label='UCM')
ax.plot(time_arr, C_fp_traj[:, 0, 0],  color=COL['fenep'], lw=2,  ls=':', label='FENE-P')
ax.set_xlabel('Time'); ax.set_ylabel(r'$C_{xx}$')
ax.set_title(r'3D Conformation Tensor Component $C_{xx}$ vs Time')
ax.legend()
plt.tight_layout(); plt.savefig('results/plots/fig6_Cxx.png'); plt.close()

# Fig 7: Error histogram
fig, ax = plt.subplots(figsize=(8, 4))
err = (Y_pred - Y_true).flatten()
ax.hist(err, bins=80, color=COL['nn'], alpha=0.8, edgecolor='white')
ax.axvline(0, color='black', lw=1.5, ls='--')
ax.set_xlabel('Prediction Error')
ax.set_ylabel('Count')
ax.set_title('3D Error Distribution')
plt.tight_layout(); plt.savefig('results/plots/fig7_error_histogram.png'); plt.close()

# Fig 8: Frobenius dist
fig, ax = plt.subplots(figsize=(8, 4))
frob_v = np.linalg.norm(Y_true - Y_pred, axis=1) / (np.linalg.norm(Y_true, axis=1) + 1e-12)
ax.hist(frob_v * 100, bins=60, color=COL['fenep'], alpha=0.8, edgecolor='white')
ax.set_xlabel('Relative Frobenius Error (%)')
ax.set_ylabel('Count')
ax.set_title('3D Frobenius Error Distribution')
plt.tight_layout(); plt.savefig('results/plots/fig8_frobenius_dist.png'); plt.close()
...
ax.set_title('A — 3D Training Loss'); ax.set_xlabel('Epoch'); ax.legend(fontsize=9)
...
ax.set_title('C — 3D Error Histogram'); ax.set_xlabel('Error')

ax = fig.add_subplot(gs[1, :2])
ax.plot(time_arr, tr(C_ref_traj), color=COL['ref'],   lw=2.5, label='3D BD Ref')
ax.plot(time_arr, tr(C_nn_traj),  color=COL['nn'],    lw=2,   label='3D N2X NN')
...
ax.set_title(r'D — 3D tr(C) Shear $\dot\gamma=1$'); ax.legend(fontsize=9)
...
fig.suptitle('N2X 3D — Complete Results Dashboard', fontsize=14, fontweight='bold')
plt.savefig('results/plots/fig9_dashboard.png'); plt.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60)
print("  FINAL 3D RESULTS SUMMARY")
print("="*60)
print(f"  Dataset samples   : {X.shape[0]:,}")
print(f"  Network size      : 13 → 128 → 128 → 64 → 6")
